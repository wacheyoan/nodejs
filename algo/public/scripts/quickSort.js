...
function triCustom() {
    ...
    for (let j = 0; j < nbBuckects; j++)
        prefixSums[j] = 0;
    for (let j = 0; j < len; j++)
        prefixSums[numbers[j]]++;
    for (let j = 1; j < nbBuckects; j++)
...
function constructionPossible(objectif, arr) {
...
function constructionPossible2(objectif, // Mon objectif courant
